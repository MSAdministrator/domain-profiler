...
class TestDNSSEC:
            ...
            with patch.object(d, "_fetch_with_rrsig", return_value=response):
                with patch.object(
                    ...
                    with patch("dns.dnssec.validate", return_value=None):
                        with patch.object(d, "_validate_ds", return_value=True):
                            result = d.validate("example.com")
    ...
    def test_dnskey_valid_but_no_matching_ds_is_bogus(self):
        """Self-signed keys with no parent DS anchor = island of trust = bogus."""
        d = DNSSEC()
        dnskey_rrset = [self._mock_key(13)]
        response = Mock()
        with patch.object(d, "_resolver_ad_flag", return_value=False):
            with patch.object(d, "_fetch_with_rrsig", return_value=response):
                with patch.object(
                    ...
                    with patch("dns.dnssec.validate", return_value=None):
                        with patch.object(d, "_validate_ds", return_value=False):
                            result = d.validate("example.com")
